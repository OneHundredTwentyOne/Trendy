    		...
    		pg.connect(database,function(err,client,done){
		      if(err) {
    				return console.error('could not connect to postgres', err);
  					...
  					console.log('Connected to database');
  					 var query = "INSERT INTO Users (Username,RealName, Password) VALUES (Username='%USERNAME%' AND RealRame = '%NAME%' AND Password='%PASSWORD%');".replace("%USERNAME%", USERNAME).replace("%NAME%", NAME).replace("%PASSWORD%", PASSWORD);
  					  client.query(query, function(error, result){
